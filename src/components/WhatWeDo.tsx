            ...
            return (
              ...
              >
                ...
                <div className={`md:col-span-8 ${reversed ? "md:order-1" : ""}`}>
                  ...
                  >
                    {v.price && (
                      <span className="inline-flex items-center bg-navy px-4 py-2 text-[13px] font-semibold text-white">
                        {v.price}
                      </span>
                    )}
                    <span className="text-[13px] text-steel">{v.priceDetail}</span>
                  </div>

                  {v.postAward && (
                    <div
                      className={`mt-6 flex flex-wrap gap-x-8 gap-y-2 border-t border-lightgray pt-5 ${
                        reversed ? "md:justify-end" : ""
                      }`}
                    >
                      {v.postAward.map((item) => (
